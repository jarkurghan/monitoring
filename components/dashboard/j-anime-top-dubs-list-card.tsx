...
import { AnimeTopDub } from "@/services/anime";
...
interface AnimeTopDubsListCardProps {
    data: AnimeTopDub[];
}

export function AnimeTopDubsListCard({ data }: AnimeTopDubsListCardProps) {
    return (
        <Card className="bg-card border-border h-full">
            <CardHeader className="flex flex-row items-center justify-between pb-2">
                <CardTitle className="text-base font-medium">Top dublar</CardTitle>
            </CardHeader>
                ...
                {data.map((row, index) => (
                    <div key={index} className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <p className="text-sm font-medium truncate max-w-[8rem] sm:max-w-[24rem] lg:max-w-[6rem] xl:max-w-[11rem]" title={row.dub_name}>
                                {row.dub_name}
                            </p>
                        </div>
