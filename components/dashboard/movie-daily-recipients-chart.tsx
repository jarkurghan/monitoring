...
type MovieDailyRecipientsStat = {
    date: string;
    movies: number;
    dateSlice?: string;
};

interface MovieDailyRecipientsChartProps {
    data: MovieDailyRecipientsStat[];
    color: string;
}

export function MovieDailyRecipientsChart({ data, color }: MovieDailyRecipientsChartProps) {
    ...
    return (
        <Card className="bg-card border-border h-full">
            <CardHeader className="flex flex-row items-center justify-between pb-2">
                <div>
                    <CardTitle className="text-base font-medium">Yangi multfilmlar soni</CardTitle>
                </div>
                    ...
                    <div className="ml-auto flex items-center gap-4">
                        <div className="flex items-center gap-2">
                            ...
                            <div>
                                ...
                                <p className="text-xs font-medium">yangi multfilmlar soni</p>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={data} barGap={2}>
                            ...
                            <Tooltip
                                ...
                                formatter={(value: number) => [value.toLocaleString(), "Yangi multfilmlar soni"]}
                                labelFormatter={(_, payload) => payload?.[0]?.payload?.date || ""}
                            />
                            <Bar dataKey="movies" radius={[4, 4, 0, 0]} maxBarSize={20}>
