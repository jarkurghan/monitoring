...
export function InstaTopGroupsBarChart({ data }: InstaTopGroupsBarChartProps) {
    const normalized = (data ?? []).map((group) => ({ ...group, sliced: group.group_name.slice(0, 7) })).slice(0, 7);
            ...
            <CardContent className="space-y-3">
                <ResponsiveContainer width="100%" height={300}>
                    <BarChart data={normalized} margin={{ top: 5, right: 5, left: 5, bottom: 5 }}>
                        ...
                        <Bar dataKey="total_count" name="foydalanishlar soni" fontSize={11} fill="#8884d8" />
